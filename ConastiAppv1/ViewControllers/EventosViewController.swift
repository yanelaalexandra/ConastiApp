...
class EventosViewController: BaseViewController, UICollectionViewDelegate, UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cellEve", for: indexPath)
        //----------------
    ...
    func fetchUserData(_ uuid:String){
        DispatchQueue.main.async {
            Alamofire.request("https://conasti-app-v3-sebasraureyes.c9users.io/noticia/usuario/eventos/\(uuid)").responseJSON(completionHandler: {(response) in
