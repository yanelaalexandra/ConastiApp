...
class MenuViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    @IBOutlet weak var MyButtonCloseBtn: UIButton!
    ...
    @IBAction func MyButtonCloseTapped(_ button: UIButton) {
        
        btnMenu.tag = 0
        
        if (self.delegate != nil) {
            var index = Int32(button.tag)
            if(button == self.MyButtonCloseBtn){
                index = -1
            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let imgIcon : UIImageView = cell.contentView.viewWithTag(1610) as! UIImageView
