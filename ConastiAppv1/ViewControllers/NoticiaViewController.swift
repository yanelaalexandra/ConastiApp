...
class NoticiaViewController: BaseViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cellNoti", for: indexPath)
        //----------------
    ...
    func fetchUserData(_ uuid:String){
        DispatchQueue.main.async {
            Alamofire.request("https://conasti-app-v3-sebasraureyes.c9users.io/noticia/usuario/noticias/\(uuid)").responseJSON(completionHandler: {(response) in
